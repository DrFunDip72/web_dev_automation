...
def bs_upload_pdf(page, sCleanID, exportFolder) :
    ...
    print(pdf_path)
    sleep(3)
    page.locator('input[type="file"][data-choose="Choose"]').set_input_files(pdf_path)
    sleep(5)
    page.locator("form").filter(has_text=f"New Attachment").locator("button[name=\"action-publish\"]").click()
    sleep(5)
    page.get_by_text("Back", exact=True).click()
    sleep(3)
    firstNum, lastNum = sCleanID.split('-') # splits the sCleanID so I can get the last digits to be able to link it
    page.locator(f'a:has-text("{sCleanID}-sell-sheet.pdf")').first.click()
    page.get_by_role("button", name="Save & Close").click()
